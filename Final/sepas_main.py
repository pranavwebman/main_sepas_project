...
def start_sos():
    ...
    try:
        ...
        payload = f"{DEVICE_NAME}:SOS_ALERT"
...
def check_sensors():
    ...
    if button.value() == 0:
        ...
        start_sos()
        e.send(broadcast_mac, f"{DEVICE_NAME}:MANUAL_SOS".encode())
        return
        
    if fire_sensor.value() == FIRE_TRIGGER_LEVEL:
        if not fire_active:
            print("FIRE DETECTED!"); fire_active = True; start_sos();
            e.send(broadcast_mac, f"{DEVICE_NAME}:FIRE".encode())   # add this line
            return
    ...
    if sound_sensor.value() == IMPACT_TRIGGER_LEVEL:
        if not impact_active and (now - last_trigger_time) > DEBOUNCE_MS:
            ...
            e.send(broadcast_mac, f"{DEVICE_NAME}:IMPACT".encode())
...
def handle_request(request):
    ...
    if path == '/mansos':
        ...
        e.send(broadcast_mac, f"{DEVICE_NAME}:PANIC_ON".encode())
        return "Panic ON"
    elif path == '/sound':
        buzzer.value(1)
        red.duty(1000)
