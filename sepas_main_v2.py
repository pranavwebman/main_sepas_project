...
def update_sos():
    ...
    # S Part (Steps 0 and 4) [cite: 3, 11]
    if sos_step == 0 or sos_step == 4:
        ...
        if sos_substep < len(durations):
            if sos_timer == 0 or (now - sos_timer >= durations[sos_substep]):
                # Toggle logic based on your original buzzer pattern [cite: 3, 4, 5]
                if buzzer.value() == 0:
                    buzzer.value(1)
                    led.on()
                else:
    ...
    # Pauses (Steps 1 and 3) [cite: 6, 10]
...
server.bind(('0.0.0.0', 8069)) # [cite: 32]
server.listen(5)
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                if request:
                    response = handle_request(request) # [cite: 34]
                    if response is None:
                        html = HTML.replace("{ip}", ip_address)
                        client.send(b"HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n" + html.encode())
                    elif response != "Unknown":
                        client.send(b"HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\n" + response.encode()) # [cite: 35]
            except:
                pass # [cite: 36]
            finally:
                client.close()
